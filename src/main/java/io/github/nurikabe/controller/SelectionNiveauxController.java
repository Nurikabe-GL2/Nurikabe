...
import io.github.nurikabe.backend.level.Levels;
...
public class SelectionNiveauxController extends VBox {
    ...
    private void refreshLevels() {
        ...
        List <VBox> maListe= Levels.LEVELS.stream().filter(level -> difficulties.contains(level.getDifficulty())).map(level -> {
            VBox mvbox = new VBox();
            Label mLabel = new Label();
            GridPane grille = new GridPane();
            grille.setPrefSize(100,100);
            mvbox.getChildren().add(grille);
            mLabel.setText(level.getDifficulty().getDisplayName()+ " : " + level.getName() + " ( " + level.getWidth() + "x" + level.getHeight() + " )");
            mvbox.getChildren().add(mLabel);
            return mvbox;
        }).toList();

        puzzlesTilePane.getChildren().setAll(maListe);
    }
